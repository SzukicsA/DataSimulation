...
It creates a df with all possible combionations of variable values
- Can you explain what `purrr::pmap()` does (in the context of the simulation workflow above)?
It applies a functions to a list of multiple variables
...
The more iterations the higher the power
...
# Welch dependencies
analyse_welch_data <- function(data) {
  
  
  require(effsize)
  
  res_welch_test <- t.test(formula = score ~ condition, 
                       data = data,
                       var.equal = TRUE,
                       alternative = "two.sided")
  
  res <- tibble(p_welch    = res_welch_test$p.value)
...
simulation_summary <- simulation |>
  ...
  group_by(n_control,
           n_intervention) |>
  summarize(power_student = mean(p_students < .05),
            .groups = "drop")

simulation_welch_summary <- simulation_welch |>
  unnest(analysis_welch_results) |>
  mutate(n_control = as.factor(n_control),
         n_intervention = as.factor(n_intervention)) |>
  group_by(n_control,
           n_intervention) |>
  summarize(power_welch = mean(p_welch < .95),
            .groups = "drop")
