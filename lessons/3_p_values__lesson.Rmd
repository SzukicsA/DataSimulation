...
tibble(p = rbeta(n = 100000, shape1 = 5, shape2 = 5)) |>  
  plot_p_values()   #the p-values distribution never looks like this instead it looks more like the following dist.
...
generate_data <- function(n_control,                  #Input variables
  ...
  return(data)  # this is so that the only the results is returned and the rest of the code is not returned because its not needed
}
...
analyse_data <- function(data) {
  ...
  res_t_test <- t.test(formula = score ~ condition, #condition is a factor with two lvls
                       data = data,
                       var.equal = FALSE,
...
experiment_parameters_grid <- expand_grid(   #creates all possible permutations* (combinations of the given variables)
  n_control = 50,
  n_intervention = 50,
  mean_control = 0,
